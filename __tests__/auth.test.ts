...
describe('Auth Middleware', () => {
  ...
  const originalApiKey = process.env.AGENTSHIELD_API_KEY;

  beforeEach(() => {
    // Reset env before each test
    process.env.AGENTSHIELD_API_KEY = originalApiKey;
  ...
  describe('getApiKey()', () => {
    test('returns the env variable value', () => {
      process.env.AGENTSHIELD_API_KEY = 'test-secret-key-123';
  ...
  describe('validateApiKey()', () => {
    test('returns null in development mode (NODE_ENV !== production)', () => {
      process.env.NODE_ENV = 'development';
      process.env.AGENTSHIELD_API_KEY = 'some-key';
    ...
    test('returns 401 response when in production with no API key provided', () => {
      process.env.NODE_ENV = 'production';
      process.env.AGENTSHIELD_API_KEY = 'required-key';
      const request = new NextRequest('http://localhost:3000/api/test');
      const result = validateApiKey(request);
    ...
    test('returns 401 when wrong API key provided', () => {
      process.env.NODE_ENV = 'production';
      process.env.AGENTSHIELD_API_KEY = 'correct-key';
      const request = new NextRequest('http://localhost:3000/api/test', {
    ...
    test('returns null in production when AGENTSHIELD_API_KEY is not set (unprotected)', () => {
      process.env.NODE_ENV = 'production';
      delete process.env.AGENTSHIELD_API_KEY;
      const request = new NextRequest('http://localhost:3000/api/test');
      const result = validateApiKey(request);
