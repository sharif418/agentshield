      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-1">
            <Button variant="outline" size="icon" className="h-7 w-7" onClick={() => setZoom(Math.max(0.5, zoom - 0.1))}>
              <ZoomOut className="h-3 w-3" />
            </Button>
            <span className="text-xs text-muted-foreground w-10 text-center">{Math.round(zoom * 100)}%</span>
            <Button variant="outline" size="icon" className="h-7 w-7" onClick={() => setZoom(Math.min(2, zoom + 0.1))}>
      ...
      <Card className="border-0 shadow-sm">
        <CardContent className="p-0 overflow-hidden">
          {isLoading ? (
            ...
            <div className="flex items-center justify-center h-96 text-muted-foreground text-sm">
              No trace data available for graph visualization
            </div>
          ) : (
            <div
            ...
            >
              ...
              >
                ...
                {/* Edges with animated dashes for policy decisions */}
                {edges.map((edge, i) => {
